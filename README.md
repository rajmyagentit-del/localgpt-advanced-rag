...
| 22 | Live evaluation/regression dashboard (Hard tier) | New `eval_runs` table + `record_eval_run`/`get_eval_run_history` persist every `eval/run_eval.py` run (auto-detects the git commit SHA, or `GITHUB_SHA` in CI); new `GET /v1/eval/runs` and `GET /v1/eval/runs/latest` expose the history; new `/eval-dashboard` page (Recharts trend chart, per-metric cards, recent-runs table) visualizes it, reusing the app's existing design tokens rather than introducing a new visual language. While building this, found and fixed the most significant bug of the whole session - see below. |
...
**The single most significant bug of the entire session, found while building the eval dashboard (item 22):** the frontend (`src/lib/api.ts`) has been silently broken against the real backend since the FastAPI migration (item 9) - all 21 of its API calls hit unversioned paths (`/health`, `/chat`, `/sessions`, ...), while `backend/app.py` has served everything under `/v1/` this entire time. The frontend only ever worked against the old, now-deprecated `backend/server.py`, which had no version prefix - meaning the actual running app would have returned 404 on every single request since item 9 landed. Verified all 17 unique path suffixes used across the file match the real FastAPI routes exactly, then fixed it with a single line (`API_BASE_URL` now includes `/v1`) rather than touching all 21 call sites individually. This is exactly the kind of gap that automated backend tests (all 190 of them, in this case) can never catch on their own - they never touch the frontend at all - which is itself a useful, honest lesson about the limits of this fork's test coverage.
...
Every run is saved automatically (see `--no-save-to-db` to skip) and viewable at **`/eval-dashboard`** once the frontend is running - a trend chart of each metric across runs, a pass/fail status, and a table of recent runs. With no runs recorded yet, the page shows a clear empty state with the exact command to run.
...
- 🟡 **Hard tier (5/5 complete):** GraphRAG completion, self-correcting agentic loop, PostgreSQL migration, multi-tenant isolation, live evaluation/regression dashboard.

**All 23 items from the original roadmap are now complete.** See the "About This Fork" section at the top for the honest accounting of what that actually meant in practice - including the real bugs found along the way, several of them significant, none of them hidden.
