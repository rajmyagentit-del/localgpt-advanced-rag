...
from rag_system.retrieval.graph_retriever import (
    GraphRetriever,  # noqa: F401 (re-exported for backward compatibility)
)
...
class MultiVectorRetriever:
    ...
    def retrieve(
        self, text_query: str, table_name: str, k: int, reranker=None
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            ...
            def _run_fts():
                ...
                return tbl.search(query=fts_query, query_type="fts").limit(fts_k).to_df()
            ...
            dedup_subset = (
                ...
                else (["chunk_id"] if "chunk_id" in combined.columns else None)
            )
            if dedup_subset:
            ...
            for _, row in results_df.iterrows():
                ...
                try:
                    if raw_score is None or (
                        isinstance(raw_score, float) and math.isnan(raw_score)
                    ):
                        raw_score = 0.0
                except Exception:
                    raw_score = 0.0
