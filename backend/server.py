...
class ChatHandler(http.server.BaseHTTPRequestHandler):
        ...
        try:
            ...
            if not self.ollama_client.is_ollama_running():
                self.send_json_response(
                    {"error": "Ollama is not running. Please start Ollama first."}, status_code=503
                )
                return
    ...
    def handle_get_sessions(self):
        ...
        try:
            ...
            self.send_json_response({"error": f"Failed to get sessions: {str(e)}"}, status_code=500)
    ...
    def handle_cleanup_sessions(self):
        ...
        try:
            ...
            self.send_json_response(
                {"error": f"Failed to cleanup sessions: {str(e)}"}, status_code=500
            )
    ...
    def handle_get_session(self, session_id: str):
        ...
        try:
            ...
            self.send_json_response({"session": session, "messages": messages})
        except Exception as e:
            self.send_json_response({"error": f"Failed to get session: {str(e)}"}, status_code=500)
    ...
    def handle_get_session_documents(self, session_id: str):
        ...
        try:
            ...
            self.send_json_response(
                {"session": session, "files": filenames, "file_count": len(docs)}
            )
        except Exception as e:
            self.send_json_response(
                {"error": f"Failed to get documents: {str(e)}"}, status_code=500
            )
    ...
    def handle_create_session(self):
        ...
        try:
            ...
            self.send_json_response({"session": session, "session_id": session_id}, status_code=201)

        except json.JSONDecodeError:
            self.send_json_response({"error": "Invalid JSON"}, status_code=400)
        except Exception as e:
            self.send_json_response(
                {"error": f"Failed to create session: {str(e)}"}, status_code=500
            )
    ...
    def handle_session_chat(self, session_id: str):
        ...
        try:
            ...
            logger.warning(
                f"⚠️  Client disconnected during RAG processing for query: '{message[:30]}...'"
            )
        except json.JSONDecodeError:
            self.send_json_response({"error": "Invalid JSON"}, status_code=400)
        except Exception as e:
...
Respond with exactly one word: USE_RAG or DIRECT_LLM"""
            ...
            for file_item in files:
                if file_item.filename:
                    ...
                    uploaded_files.append(
                        {"filename": file_item.filename, "stored_path": absolute_file_path}
                    )
    ...
    def handle_index_documents(self, session_id: str):
        ...
        try:
            ...
            if not file_paths:
                self.send_json_response(
                    {"message": "No documents to index for this session."}, status_code=200
                )
                return
            ...
            rag_response = requests.post(
                rag_api_url, json={"file_paths": file_paths, "session_id": session_id}
            )
            ...
            if rag_response.status_code == 200:
                ...
                try:
                    db.update_index_metadata(
                        session_id, idx_meta
                    )  # session_id used as index_id in text table naming
                except Exception as e:
                ...
                logger.error(
                    f"❌ RAG API indexing failed ({rag_response.status_code}): {error_info}"
                )
                self.send_json_response(
                    {"error": f"Indexing failed: {error_info}"}, status_code=500
                )
    ...
    def handle_get_models(self):
        ...
        try:
            ...
            self.send_json_response(
                {"generation_models": generation_models, "embedding_models": embedding_models}
            )
        except Exception as e:
            self.send_json_response({"error": f"Could not list models: {str(e)}"}, status_code=500)
        ...
        if "files" in form:
            ...
            for f in files:
                if f.filename:
                    ...
                    uploaded_files.append(
                        {"filename": f.filename, "stored_path": os.path.abspath(path)}
                    )
        if not uploaded_files:
            self.send_json_response({"error": "No files uploaded"}, status_code=400)
            return
        self.send_json_response(
            {"message": f"Uploaded {len(uploaded_files)} files", "uploaded_files": uploaded_files}
        )
    ...
    def handle_build_index(self, index_id: str):
        try:
            ...
            if rag_resp.status_code == 200:
                ...
                self.send_json_response({"response": rag_resp.json(), **meta_updates})
            else:
                ...
                if err_text and "already exists" in err_text:
                    ...
                    self.send_json_response(
                        {"message": "Index already built – skipping rebuild.", "note": err_text}
                    )
                else:
                    self.send_json_response(
                        {"error": f"RAG indexing failed: {rag_resp.text}"}, status_code=500
                    )
        except Exception as e:
            self.send_json_response({"error": str(e)}, status_code=500)
    ...
    def handle_delete_index(self, index_id: str):
        ...
        try:
            ...
            if deleted:
                self.send_json_response(
                    {"message": "Index deleted successfully", "index_id": index_id}
                )
            else:
                self.send_json_response({"error": "Index not found"}, status_code=404)
        except Exception as e:
    ...
    def handle_rename_session(self, session_id: str):
        ...
        try:
            ...
            self.send_json_response(
                {"message": "Session renamed successfully", "session": updated_session}
            )
            ...
            self.send_json_response(
                {"error": f"Failed to rename session: {str(e)}"}, status_code=500
            )
